...
* I used an arbitrary upper file size limit of 1 MB
* I set rules for the password so that I do not have deal with special characters that the user can input. This enabled me to write something quickly, as the password is meant to be used directly on the download URL, and I did not want to deal with time-consuming santization code - which are all very easy, but spending time with that was not the point of this app.
